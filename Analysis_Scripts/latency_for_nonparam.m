function [res,cfgNew] = latency_for_nonparam(avgs, cfg, sign)
...
function [cfg,cfgNew]=checkConfig(cfg,avgs)
    ...
    if isfield(cfg,'times')
        ...
        if length(cfg.times)==2
            cfg.times=linspace(cfg.times(1),cfg.times(2),n_samples);
        elseif length(cfg.times) ~= n_samples
...
%                             checkFields{i},cfg.(checkFields{i}),cfg.timeFormat,cfgNew.(checkFields{i}),cfg.timeFormat);
        ...
        for i=1:length(checkFields)
            if isfield(cfg,checkFields{i}) && isnumeric(cfg.(checkFields{i}))
                if any(mod(cfg.(checkFields{i}),1)) || (isempty(regexp(checkFields{i},'Width','once')) && (any(cfg.(checkFields{i})<1) || any(cfg.(checkFields{i})>n_samples)))
...
function peak=peakDetection(cfg,ERP,peakLat)
    ...
    if length(ERPcut)>2
        [~,lats] = findpeaks(ERPcut,'SORTSTR', 'descend');
        if numel(lats)<1
